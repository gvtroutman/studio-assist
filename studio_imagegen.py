...
def add_face_finder(graph, sam3, pixels=None):
    """Into a filled graph: SAM3's face boxes and the picture's size for
    `pixels` (by default its SaveImage's picture), as PreviewAny text (read
    by `face_boxes`)."""
    if pixels is None:
